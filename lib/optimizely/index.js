...
Optimizely.prototype.activateVariation = function(experimentKey, userId, variationId, attributes) {
  try {
    if (!this.__validateInputs(userId, attributes) || !this.__validatePreconditions(experimentKey, userId, variationId, attributes)) {
      var failedActivationLogMessage = sprintf(LOG_MESSAGES.NOT_ACTIVATING_USER, MODULE_NAME, userId, experimentKey);
      this.logger.log(LOG_LEVEL.INFO, failedActivationLogMessage);
      return null;
    }
